"""Convert Rainy Cityscapes into the shared seven-class COCO protocol.

Weather parameters and integrity counts are exposed through CLI and summaries.
"""
...
RAIN_RE = re.compile(
    r"^(?P<base>.+)_leftImg8bit_rain_alpha_(?P<alpha>[^_]+)"
    r"_beta_(?P<beta>[^_]+)_dropsize_(?P<dropsize>[^_]+)"
    r"_pattern_(?P<pattern>[^_]+)$"
)
...
def main():
    ...
    parser.add_argument("--dataset-root", type=Path, default=workspace_root / "Cityscapes_rain_val")
    parser.add_argument("--output-dir", type=Path, default=project_root / "data_cityscapes_rain")
    parser.add_argument("--name", default="cityscapes_rain_val_alpha_0.02")
    parser.add_argument("--alpha", default="0.02", help="Rain alpha split, or all")
    parser.add_argument("--beta", default="all", help="Rain beta split, or all")
    parser.add_argument("--dropsize", default="all", help="Rain dropsize split, or all")
    parser.add_argument("--pattern", default="all", help="Rain pattern split, or all")
    ...
    selected_rows = [
        ...
        if selected_value(row["alpha"], args.alpha)
        and selected_value(row["beta"], args.beta)
        and selected_value(row["dropsize"], args.dropsize)
        and selected_value(row["pattern"], args.pattern)
    ]
    ...
    alpha_counts = Counter(row["alpha"] for row in all_image_rows)
    selected_alpha_counts = Counter(row["alpha"] for row in selected_rows)
    selected_pattern_counts = Counter(row["pattern"] for row in selected_rows)
    missing_labels = []
    bad_images = []
    bad_labels = []
    ann_id = 1
        ...
        coco_images.append(
            {
                ...
                "height": height,
                "scene": row["base"],
                "style": "Cityscapes_rain_val",
                "city": row["city"],
                "alpha": row["alpha"],
                "beta": row["beta"],
                "dropsize": row["dropsize"],
                "pattern": row["pattern"],
    ...
    data = {
        "info": {
            "description": "Cityscapes_rain_val converted from gtFine polygons to COCO-style boxes",
            "label_note": "Only bicycle, bus, car, motorcycle, person, rider, truck are kept and mapped to the 7 training classes.",
            "alpha_filter": args.alpha,
            "beta_filter": args.beta,
            "dropsize_filter": args.dropsize,
    ...
    summary = {
        ...
        "categories": CATEGORIES,
        "source_alpha_counts": dict(sorted(alpha_counts.items())),
        "selected_alpha_counts": dict(sorted(selected_alpha_counts.items())),
        "selected_pattern_counts": dict(sorted(selected_pattern_counts.items())),
    ...
    print("Prepared Cityscapes_rain_val Faster R-CNN evaluation data")
    print("  alpha filter: {}".format(args.alpha))
    print("  beta filter: {}".format(args.beta))
    print("  dropsize filter: {}".format(args.dropsize))
    print("  pattern filter: {}".format(args.pattern))
